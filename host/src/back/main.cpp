...
//host buffer
float *h_result_classifier = (float *)malloc((10) * sizeof(float)); 
...
cl_mem d_classifier_weight, d_classifier_bias, d_result_classifier;
...
int main() {
  ...
  d_sample = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                       sizeof(float) * (1 * 1 * 28 * 28), sample, &status);
  ...
  //Layer 2
  d_layer2_weight = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
                       sizeof(layer2_weight), layer2_weight, &status);
  checkError(status, "Failed to create buffer d_layer2_weight");
  d_layer2_bias = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
  ...
  d_result_layer2 = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
                       sizeof(float) * (1 * 1 * 30 * 30), NULL, &status);
  ...
  d_result_layer3 = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                       sizeof(float) * (1 * 1 * 1 * 10), NULL, &status);
  checkError(status, "Failed to create buffer d_result_layer3");
	// Classifier Layer
  d_classifier_weight = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                       sizeof(classifier_weight), classifier_weight, &status);
  checkError(status, "Failed to create buffer d_classifier_weight");
  d_classifier_bias = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                       sizeof(classifier_bias), classifier_bias, &status);
  checkError(status, "Failed to create buffer d_classifier_bias");
  d_result_classifier = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                       sizeof(float) * (1 * 1 * 1 * 10), NULL, &status);
  ...
  status |= clSetKernelArg(fc, 3, sizeof(int), &(d_classifier_weight));
  status |= clSetKernelArg(fc, 4, sizeof(int), &(d_classifier_bias));
  status |= clSetKernelArg(fc, 5, sizeof(int), &(d_result_classifier));
  ...
  end_time = getCurrentTimestamp();
  status = clEnqueueReadBuffer(queue, d_result_classifier, CL_TRUE, 0, sizeof(float) * 10, h_result_classifier, 0, NULL, NULL );
...
void cleanup()
{
  ...
  clReleaseMemObject(d_classifier_weight);
  clReleaseMemObject(d_classifier_bias);
