...
#include "gesturefcnet_params.h"
#include "testimg.h"
using namespace aocl_utils;
using namespace std;

void getLabel(unsigned int class_index, char *class_label);
// Entry point.
int adxlpredict() {
// Reading PIC
float *sampleacc = (float *)malloc((784) * sizeof(float)); 
...
//host buffer
float *h_result_classifier2 = (float *)malloc((10) * sizeof(float)); 
float *h_result_layer1 = (float *)malloc((784) * sizeof(float)); 
...
cl_mem d_classifier2_weight, d_classifier2_bias, d_result_classifier2;
  ...
  d_sample = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                       sizeof(float) * (1 * 1 * 28 * 28), sampleacc, &status);
...
/*   status = clEnqueueReadBuffer(queue, d_sample, CL_TRUE, 0, sizeof(float) * 784, h_result_layer1, 0, NULL, NULL );
  ...
  end_time = getCurrentTimestamp();
  status = clEnqueueReadBuffer(queue, d_result_classifier2, CL_TRUE, 0, sizeof(float) * 10, h_result_classifier2, 0, NULL, NULL );
  ...
  getLabel(class_index,class_label2);
